/*
 * Copyright 2005-2013 hengtiansoft.com. All rights reserved.
 * Support: http://www.hengtiansoft.com
 * License: http://www.hengtiansoft.com/license
 * 
 * JavaScript - Common
 * Version 1.0_beta
 */
...
(function($) {
	...
	$.message = function() {
		...
		if ($message == null) {
			$message = $('<div class="ui-message"><div class="ui-msg-content ui-msg-' + message.type + '"></div></div>');
		...
		$message.children("div").removeClass("ui-msg-warn ui-msg-error ui-msg-success").addClass("ui-msg-" + message.type).html(message.content);
	...
	$.dialog = function(options) {
		...
		$dialog.css({"width": settings.width, "height": settings.height, "margin-left": - parseInt(settings.width / 2), "z-index": zIndex ++, "overflow-y": "auto"});
	...
	$.fn.extend({
		browser: function(options) {
			var settings = {
				...
				isUpload: true,
				browserUrl: xinyunlian.base + "/admin/file/browser.jhtml",
				uploadUrl: xinyunlian.base + "/admin/file/upload.jhtml",
				callback: null
			};
			$.extend(settings, options);
					...
					function createBrowserList(path, data) {
						...
						$.each(data, function(i, fileInfo) {
							...
							if (fileInfo.isDirectory) {
								iconUrl = xinyunlian.base + "/resources/admin/images/folder_icon.gif";
								title = fileInfo.name;
								...
								iconUrl = xinyunlian.base + "/resources/admin/images/file_icon.gif";
								title = fileInfo.name + " (" + Math.ceil(fileInfo.size / 1024) + "KB, " + new Date(fileInfo.lastModified).toLocaleString() + ")";
							}
						...
						$browserList.find("img").bind("click", function() {
							...
							if (isDirectory == "true") {
								...
								$dialog.next(".dialog-overlay").andSelf().remove();
							}
						});
	...
	$.stripeTable = function($target){
		//斑马纹，用于IE8
		if(window.navigator.userAgent.indexOf("MSIE 8.0")>=1){
			if($target == null) $("table.list tr:nth-child(odd)").addClass("odd-list");
			else $target.find("table.list tr:nth-child(odd)").addClass("odd-list");
		}
	}
...
!function(){
	...
	var bdata = $('body').data();
	if(bdata && bdata.msgContent !== undefined){
		$.message(bdata.msgType, bdata.msgContent);
	}
	
	// 返回按钮
	var $return = $("#return");
	if ($return.size() > 0 && ($return.attr("onclick") == null || $return.attr("onclick") == "")) {
		var listQuery = getCookie("listQuery");
		if (listQuery && listQuery != "") {
			listQuery.replace(/%3D/g, "=").replace(/%26/g, "&");
			$return.attr("onclick", "location.href='list.jhtml" + listQuery + "'");
		} else {
			$return.attr("onclick", "location.href='list.jhtml'");
		}
	}
...
(function($) {
	$.encodeHTMLSource = function(code) {
		var encodeHTMLRules = {
			"&" : "&#38;",
			"<" : "&#60;",
			">" : "&#62;",
		...
		return code ? code.toString().replace(/[&<>"'\/]/g, function(m) {
