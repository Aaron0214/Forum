...
if(typeof(KindEditor) != "undefined") {
	KindEditor.ready(function(K) {
		editor = K.create("#editor", {
			...
			langType: xinyunlian.locale,
			syncType: "form",
			filterMode: false,
			pagebreakHtml: '<hr class="pageBreak" />',
			allowFileManager: true,
			filePostName: "file",
			fileManagerJson: xinyunlian.base + "/admin/file/browser.jhtml",
			uploadJson: xinyunlian.base + "/admin/file/upload.jhtml",
			...
			formatUploadResponse: function(data){
				data.url = "/xinyunlian-admin/static/upload/image" + data.url;
			}
		});
	});
}
